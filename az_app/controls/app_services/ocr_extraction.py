...
def show_each_file_pages_images(filename):
    ...
    '''
        This function takes in param the document name and returns its pages converted to images
    '''
...
def show_cover_pages_name():
    ...
    '''
        This function just returns the cover page of documents
    '''
...
def create_first_page_images():
    ...
    '''
        To browse easily the different uploaded documents on the browser,
        This function is designed to create a cover page for all these uploaded documents
    '''
...
def create_respective_dir():
    ...
    '''
    This function is designed create special directories for each file uploaded.
    So the converted images of the document will be saved into : 'az_app/app_ressources/translator_ressources/converted_files/the_document_name'
    And the extracted text of the images of the document will be saved into : 'az_app/app_ressources/translator_ressources/txt_extracted_files/the_document_name'

    The function "os.path.splitext(convers_each_dir)[0]" returns the name of file without the extension
    '''
...
def transform_to_images():
    ...
    '''
    This function implement the conversion of the "PDF" files to "JPG Image" files
    '''
    ...
    for doc in upl_docs:
        ...
        '''
         For each document in uploaded documents, it stores the start and the end page
         and the relative path in which will be stored the image 
        '''
        ...
        for page in pdf_to_convert:
            ...
            '''
                It converts each page of the document to images, beginning from the start page given in the database 
                to that Document and stop at the end given too
            '''
...
def extract_words_from_images():
    ...
    '''
    This function implements the text'extraction of the OCR (Optical Character Recognition) PyTesseract
    '''
    ...
    for doc in upl_docs:
        ...
        '''
            First, it browses all the uploaded files in the database to get the start and end of their pages' extraction
        '''
        ...
        for img_incr in range(convers_start_page, convers_end_page+1):
            ...
            '''
                For each founded document, it browses each image file beginning from the start to the of page indicated
                and for each image, it extract all in the variable "extraction_to_data_result"
            '''

            all_text_extracted = ""

            '''
                Setting of the image and the text files name
            '''
            ...
            for i in range(len(extraction_to_data_result['text'])):
                ...
                '''
                    For each line in the extracted data of the variable "extraction_to_data_result",
                    it returns only the text recognized and appends it to the variable "all_text_extracted"
                    And at the end of the loop all the texts are inserted in file ".txt" bearing the name of the doc 
                '''
                ...
                if extraction_to_data_result['text'][i] != '':
                    ...
                    '''
                        This line avoids to save same word multiple times in the dictionnary table called "ExtractedWords"
                        of the database
                    '''
            ...
            '''
                This line updates the extraction progress of each page
            '''
            ...
            '''
                At this line we are at the end of a whole image text extraction and
                the extracted text is saved in to a ".txt" file
            '''
            txt_to_create = open(path_relative_txt_extracted+'/'+text_file_name, 'w+')

            txt_to_create.write(all_text_extracted)

            print('Operation completed : '+the_image_to_extract+ ' The progression\'status is : '+str(final_progress)+'%')
